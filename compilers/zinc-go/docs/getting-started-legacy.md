...
> **Note:** this is the older getting-started guide, kept for reference.
> The current guide lives at [getting-started.md](getting-started.md) —
> please use that instead. This file's `or { }` examples reflect a
> pre-pivot syntax; the current error handler keyword is `catch`.
