...
type Parser struct {
	...
	// decoComplex is set by parseDecorators when a decorator carries call
	// arguments (`@deco(x)`) or a dotted name (`@mod.deco`) — forms a plain
	// function decorator can't yet lower. Read right after parseDecorators.
	decoComplex bool
	// globalDecls collects generated package-level Go declarations (currently
	// the function-decorator rebinds `var f any = deco(fImpl)`) that the driver
	// emits into a separate file. They reference package-main functions, and
	// the user-facing name resolves as a dynamic value at every call site.
	globalDecls []string
}
...
type Meta struct {
	FFIModules []string
	// GlobalDecls holds generated package-level Go declarations (function
	// decorator rebinds) to emit into a separate file, or "" if none.
	GlobalDecls string
}
...
func Parse(src string) (*parser.Program, *Meta, []string) {
	...
	return prog, &Meta{FFIModules: p.ffiImports, GlobalDecls: p.globalDeclsSource()}, nil
}

// globalDeclsSource renders the collected package-level decls into a Go source
// file (package main), or "" when there are none.
func (p *Parser) globalDeclsSource() string {
	if len(p.globalDecls) == 0 {
		return ""
	}
	return "// Code generated by zincpy. DO NOT EDIT.\npackage main\n\n" +
		strings.Join(p.globalDecls, "\n") + "\n"
}
...
func (p *Parser) parseProgram() *parser.Program {
	...
	for p.cur().Kind != TEOF {
		...
		if p.isKw("def") {
			complexDeco := p.decoComplex
			if fn := p.parseDef(); fn != nil {
				if len(decorators) > 0 {
					if complexDeco {
						// @deco(args) / @mod.deco need the decorator's full value
						// expression, which we don't reconstruct yet.
						p.errf(Token{Line: fn.Line}, "decorator with arguments or a dotted name on a function is not yet supported")
					} else {
						p.applyFnDecorators(fn, decorators)
					}
				}
...
// applyFnDecorators lowers `@a @b def f(...)` to: keep the original body under a
// hidden Go name `zincpyUndecorated_f`, and rebind the user-facing name `f` to
// the decorator application `a(b(zincpyUndecorated_f))` as a package-level var
// (`var f any = a(b(zincpyUndecorated_f))`, emitted by the driver). `f` is then
// a dynamic value, so every call site `f(args)` routes through zincpyCallDynamic
// — the same machinery that powers returned closures. Decorators nearest the
// def apply first (bottom-up), matching Python.
func (p *Parser) applyFnDecorators(fn *parser.FnDecl, decorators []string) {
	orig := fn.Name
	impl := "zincpyUndecorated_" + orig
	fn.Name = impl

	expr := impl
	for i := len(decorators) - 1; i >= 0; i-- {
		expr = decorators[i] + "(" + expr + ")"
	}
	p.globalDecls = append(p.globalDecls, "var "+orig+" any = "+expr)

	// The user-facing name is now a dynamic value, not a direct Go function, so
	// calls to it dispatch through zincpyCallDynamic. Drop its function registry
	// entries (registered by parseDef under the original name).
	p.declare(orig, tDynamic)
	delete(p.fnRet, orig)
	delete(p.defParams, orig)
}
