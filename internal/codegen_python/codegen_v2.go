...
// emitV2ChainedCollection emits a collection chain using the best backend.
func (g *Generator) emitV2ChainedCollection(e *parser.CallExpr) string {
	if g.OptimizeBackend == "polars" {
		return g.emitPolarsChain(e)
	}
...
// emitPolarsChain generates a Polars lazy frame pipeline for chained collection ops.
...
func (g *Generator) emitPolarsChain(e *parser.CallExpr) string {
	g.neededImports["polars as pl"] = true

	// Collect chain links from bottom to top
	...
	for {
		sel, ok := current.Callee.(*parser.SelectorExpr)
		if !ok || !collectionMethods[sel.Field] {
			rootExpr = g.emitV2Expr(current.Callee)
			break
		}
		chain = append([]chainLink{{method: sel.Field, call: current}}, chain...)
		if innerCall, ok := sel.Object.(*parser.CallExpr); ok {
			current = innerCall
		} else {
			rootExpr = g.emitV2Expr(sel.Object)
			break
		}
	}

	// Build Polars pipeline
	result := fmt.Sprintf("pl.DataFrame(%s).lazy()", rootExpr)
	terminalOp := ""
	...
	for _, link := range chain {
		...
		case "filter":
			if len(link.call.Args) == 1 {
				pred := g.emitPolarsFilter(link.call.Args[0])
				result += fmt.Sprintf(".filter(%s)", pred)
			}
		case "map":
			if len(link.call.Args) == 1 {
				if lam, ok := link.call.Args[0].(*parser.LambdaExpr); ok && lam.Expr != nil {
					col := g.extractPolarsColumn(lam)
					if col != "" {
						result += fmt.Sprintf(".select(%q)", col)
					} else {
						// Complex map — fall back to with_columns
						result += fmt.Sprintf(".with_columns(%s)", g.emitV2Expr(link.call.Args[0]))
					}
				}
			}
		case "sort_by":
			if len(link.call.Args) == 1 {
				col := ""
				if lam, ok := link.call.Args[0].(*parser.LambdaExpr); ok {
					col = g.extractPolarsColumn(lam)
				}
				reverse := "False"
				for _, na := range link.call.NamedArgs {
					if na.Name == "reverse" {
						reverse = g.emitV2Expr(na.Value)
					}
				}
				if col != "" {
					result += fmt.Sprintf(".sort(%q, descending=%s)", col, reverse)
				}
			}
		case "take":
			if len(link.call.Args) == 1 {
				result += fmt.Sprintf(".head(%s)", g.emitV2Expr(link.call.Args[0]))
			}
		case "skip":
			if len(link.call.Args) == 1 {
				result += fmt.Sprintf(".tail(-%s)", g.emitV2Expr(link.call.Args[0]))
			}
		case "sum":
			terminalOp = "sum"
			result += ".sum()"
		...
		case "distinct":
			result += ".unique()"
		default:
			// Unsupported in Polars — fall back
			result += fmt.Sprintf(".collect().to_series().to_list()")
			return result
		}
	}
	...
	if terminalOp != "" {
		// Aggregate — returns single value
		result += ".collect().item()"
	} else {
		// Returns a list
		result += ".collect().to_dicts()"
	}

	return result
}

// emitPolarsFilter converts a lambda predicate to a Polars expression.
// o -> o["status"] == "active" → pl.col("status") == "active"
func (g *Generator) emitPolarsFilter(arg parser.Expr) string {
	lam, ok := arg.(*parser.LambdaExpr)
	if !ok || lam.Expr == nil {
		return g.emitV2Expr(arg)
	}

	return g.exprToPolars(lam.Expr, lam.Params[0].Name)
}

// exprToPolars converts an expression to Polars expression syntax.
// Replaces param["field"] with pl.col("field"), operators stay as-is.
func (g *Generator) exprToPolars(e parser.Expr, paramName string) string {
	switch e := e.(type) {
	case *parser.BinaryExpr:
		left := g.exprToPolars(e.Left, paramName)
		right := g.exprToPolars(e.Right, paramName)
		op := e.Op
		switch op {
		case "&&":
			op = "&"
		...
		return fmt.Sprintf("(%s %s %s)", left, op, right)
	case *parser.IndexExpr:
		// param["field"] → pl.col("field")
		if ident, ok := e.Object.(*parser.Ident); ok && ident.Name == paramName {
			if str, ok := e.Index.(*parser.StringLit); ok {
				return fmt.Sprintf("pl.col(%q)", str.Value)
			}
		}
		return g.emitV2Expr(e)
	case *parser.SelectorExpr:
		// param.field → pl.col("field")
		if ident, ok := e.Object.(*parser.Ident); ok && ident.Name == paramName {
			return fmt.Sprintf("pl.col(%q)", e.Field)
		}
		return g.emitV2Expr(e)
	default:
		return g.emitV2Expr(e)
...
// extractPolarsColumn extracts a simple column access from a lambda.
// o -> o["amount"] → "amount"
// o -> o.amount → "amount"
func (g *Generator) extractPolarsColumn(lam *parser.LambdaExpr) string {
	if lam.Expr == nil || len(lam.Params) != 1 {
		return ""
	}
	paramName := lam.Params[0].Name

	switch e := lam.Expr.(type) {
	case *parser.IndexExpr:
		if ident, ok := e.Object.(*parser.Ident); ok && ident.Name == paramName {
			if str, ok := e.Index.(*parser.StringLit); ok {
				return str.Value
