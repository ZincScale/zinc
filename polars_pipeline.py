...
def create_sales_data() -> list[dict]:
    return [{"region": "east", "product": "widget", "amount": 150.0, "status": "completed"}, {"region": "west", "product": "gadget", "amount": 300.0, "status": "completed"}, {"region": "east", "product": "widget", "amount": 75.0, "status": "pending"}, {"region": "west", "product": "widget", "amount": 200.0, "status": "completed"}, {"region": "east", "product": "gadget", "amount": 450.0, "status": "completed"}, {"region": "west", "product": "gadget", "amount": 50.0, "status": "cancelled"}, {"region": "east", "product": "thing", "amount": 100.0, "status": "completed"}, {"region": "west", "product": "thing", "amount": 175.0, "status": "pending"}]

sales = create_sales_data()
print(f"Total records: {len(sales)}")
revenue = _zinc_collect(sales).filter(lambda s: (s["status"] == "completed")).map(lambda s: s["amount"]).sum()
print(f"Completed revenue: ${revenue}")
top_sales = _zinc_collect(sales).filter(lambda s: (s["amount"] > 100)).sort_by(lambda s: s["amount"], reverse=True).take(3)
print("Top 3 sales over $100:")
for sale in top_sales:
    print(f"  {sale["region"]} - {sale["product"]}: ${sale["amount"]}")
regions = [s["region"] for s in sales]
unique_regions = list(set(regions))
print(f"Regions: {unique_regions}")
pending_count = len([s for s in sales if (s["status"] == "pending")])
