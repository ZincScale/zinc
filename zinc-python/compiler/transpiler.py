...
def _braces_to_indent(lines: list[str]) -> list[str]:
    ...
    for raw_line in lines:
        ...
        # Closing brace followed by continuation. Two shapes:
        #   `} else {`, `} except {` — block-end then block-start. Pop
        #     the block, dedent, drop the `}`, and the rest is processed
        #     as a new block opener below.
        #   `} for e in entries])` — closes a dict-literal that lives
        #     inside an enclosing list/tuple. Pop the literal-brace from
        #     the stack but KEEP the `}` in the emitted line — it's the
        #     dict's actual closer and Python needs it to balance.
        if stripped.startswith("}"):
            if brace_stack and brace_stack[-1]:
                ...
                if brace_stack:
                    brace_stack.pop()
                # Don't strip the leading `}` — fall through so the line
                # gets emitted at the current indent intact, including
                # any trailing chars like `} for e in entries])`.
