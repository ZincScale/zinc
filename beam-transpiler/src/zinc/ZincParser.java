...
/**
 * Parser for the canonical .zn surface. It emits the shared {@link Ast}, so Resolve and
 * CodeGen run unchanged. The expression
 * grammar is identical (C-style operators); only declarations/statements differ:
 *   - top-level defs (including main + helpers) -> a synthetic module class
 *   - `if`/`while`/`for` take no parens; `for x in EXPR` iterates
...
final class ZincParser {
  ...
  private final List<TestDecl> tests = new ArrayList<>();

  ZincParser(List<Token> toks) {
    this(toks, "Main");
  }

  ZincParser(List<Token> toks, String fileClass) {
    this.toks = toks;
  ...
  Program parseProgram() {
    ...
    var program = new Program(imports, classes, records, actors, enums, application,
        exceptions, interfaces, instanceClasses, tests, sealeds);
    return ZincInfer.infer(program); // resolve `infer` return types from method bodies
  }
   ...
   *  a sibling module is referenced by its lowercase file name, same as the
  ...
  /** `class CounterTest : Test { void counts() { ... } }` — methods only; each
   *  zero-arg void method is an EUnit case. */
  private TestDecl parseTestBody(String name) {
    ...
    while (!check(TokKind.RBRACE) && !check(TokKind.EOF)) {
      skipPub();
      if (!(checkIdent("def") || looksLikeZincDef() || looksLikeTupleZincDef())) {
        throw new CompileError("test class " + name + " can only declare methods");
      }
      RawDef d = parseDefRaw();
      List<Param> ps = stripSelf(d.params());
      var m = new MethodDecl(d.ret(), d.name(), ps, d.body(), Set.of("public"));
      methods.add(m);
    ...
    if (testNames.isEmpty()) {
      throw new CompileError("test class " + name
          + " has no test cases: a test is a void method with no parameters");
    }
  ...
  /** `class Main : Application { child = Actor()...  void main() {..} }` — fields are the
  ...
  private RawDef parseDefRaw() {
    String ret = "infer"; // sentinel: ZincInfer resolves from the body (void if no value-return)
    String name;
    if (matchIdent("def")) {
  ...
  Expr parseExpr() {
    if (checkIdent("lambda")) return parseKeywordLambda();
    if (lambdaAhead()) return parseLambda();
    return parseTernary();
  }

  /** Lambda expression: `lambda a, b: expr` (also `lambda: expr`). */
  private Expr parseKeywordLambda() {
  ...
  private Expr parsePostfix() {
    ...
    // self.method(..) -> this.method(..), preserving older source compatibility.
   ...
   *  braces. Each hole is sub-parsed as a .zn expression. A string with no
  ...
  private Expr fstring(String text) {
    ...
    while (i < n) {
      ...
      if (c == '{') {
        ...
        lit.setLength(0);
        parts.add(new ZincParser(ZincLexer.lex(text.substring(i + 1, j))).parseExpr());
        any = true;
        i = j + 1;
  ...
  private Expr zincString(String text) {
    ...
    while (i < n) {
      ...
      if (c == '$' && i + 1 < n && text.charAt(i + 1) == '{') {
        ...
        parts.add(new StrLit(lit.toString()));
        lit.setLength(0);
        parts.add(new ZincParser(ZincLexer.lex(text.substring(i + 2, j))).parseExpr());
        any = true;
        i = j + 1;
      } else {
        lit.append(c);
  ...
  private Expr parsePrimary() {
    ...
    if (match(TokKind.LBRACKET)) { // list literal: [a, b, c]
      var elems = new ArrayList<Expr>();
    ...
    if (check(TokKind.IDENT)) {
      ...
      if (match(TokKind.LPAREN)) {
        ...
        // CapWords callee is construction; Resolve turns an Actor
